...
public class DropDown: UIView {
    ...
    public var directionTypeSet: Direction = Direction.any {
        didSet {
            let computeLayout = self.computeLayout()
            directionTypeSet = computeLayout.Direction
        }
        
    }
    
    public var directionTypeGet: Direction {
        get {
            let computeLayout = self.computeLayout()
            return computeLayout.Direction
        }
    }
     ...
     */
    @objc public dynamic var cornerRadiusDPD = DPDConstant.UI.CornerRadius {
        willSet {
    ...
    fileprivate var multiSelectedRowIndex: [Index?] = []
     ...
     */
    public convenience init(anchorView: AnchorView, selectionAction: SelectionClosure? = nil, multiSelectionAction: MultiSelectionClosure? = nil,dataSource: [String] = [], topOffset: CGPoint? = nil, bottomOffset: CGPoint? = nil, cellConfiguration: ConfigurationClosure? = nil, cancelAction: Closure? = nil, allowsMultipleSelection: Bool = false) {
        self.init(frame: .zero)
        
        self.anchorView = anchorView
        self.selectionAction = selectionAction
        self.multiSelectionAction = multiSelectionAction
...
extension DropDown: UITableViewDataSource, UITableViewDelegate {
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if allowsMultipleSelection {
            //TODO: Custom To MultiSelection
            if multiSelectedRowIndex.contains(where: {$0 == selectedRowIndex}){
                if multiSelectedRowIndex.count == 1 {
                    multiSelectedRowIndex = []
                } else {
                    let indexRemove = multiSelectedRowIndex.index(where: {$0! == selectedRowIndex!})
                    multiSelectedRowIndex.remove(at: indexRemove!)
                }
                deselectRow(at: selectedRowIndex)
            } else {
                multiSelectedRowIndex.append(selectedRowIndex!)
                
                selectRow(at: selectedRowIndex)
            }
            
            let arrStringReturn = multiSelectedRowIndex.map({dataSource[$0!]})
            
            multiSelectionAction?(multiSelectedRowIndex as! [Index], arrStringReturn)
        } else {
