...
extension UIView{
    ...
    func fadeOut(duration: TimeInterval = 1.0, delay: TimeInterval = 0.0, completion: @escaping (Bool) -> Void = {(finished: Bool) -> Void in}) {
        UIView.animate(withDuration: duration, delay: delay, options: [UIViewAnimationOptions.curveEaseIn, UIViewAnimationOptions.repeat], animations: {
            self.alpha = 0.7
        }, completion: completion)
    }
    
    func fadeAnimation(duration: TimeInterval = 1.0) {
        UIView.animateKeyframes(withDuration: duration, delay: 0.0, options: [.autoreverse, .repeat], animations: {
            self.alpha = 0.7
        }) { (finished) in
